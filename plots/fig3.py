...
def stack_metric(logs: list[dict], key: str) -> np.ndarray:
    ...
    for i, l in enumerate(logs):
        ...
        a = np.asarray(l[key], dtype=np.float32)
        arrs.append(a)
...
def main():
    ...
    for m, lab in zip(METHODS, LABELS):
        ...
        # ---- FIG 3 METRIC ----
        Y = stack_metric(logs, "avg_pl_selected")  # [S, T], in dB

        plot_mean_std(ax, Y, lab)
        print(f"[{lab}] runs: {len(paths)}")

    ax.set_xlabel("FL Round")
    ax.set_ylabel("Avg Pathloss of Selected UAVs (dB)")

    # Lower pathloss = better channel, so visually it helps if the y-axis is inverted.
    # Comment this out if you prefer normal axis.
    ax.invert_yaxis()
    ...
    out_path = os.path.join(
        RESULTS_DIR,
        f"fig3_avg_pl_selected_{DATA_MODE}_{ENV_TAG}_wireless{WIRELESS_ON}.png"
    )
    plt.tight_layout()
