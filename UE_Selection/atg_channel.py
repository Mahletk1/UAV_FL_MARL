...
def avg_pathloss_db(d, plos, fc, alpha, eta1_db, eta2_db, c=3e8):
    """
    Average ATG path loss in dB (LoS/NLoS weighted)
    """
    # Free-space path loss (dB)
    FSPL_db = 20 * np.log10(4 * np.pi * fc * d / c)

    # Large-scale path loss in dB
    PL_LoS_db = FSPL_db + 10 * alpha * np.log10(d) + eta1_db
    PL_NLoS_db = FSPL_db + 10 * alpha * np.log10(d) + eta2_db
