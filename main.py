...
# ---- Initialize scenario (outside the FL loop) ----
    ...
    for r in range(args.round):
        ...
        # 3) Packet success (PER model)
        successful_users = []
        for idx in idxs_users:
            if np.random.rand() < p_succ[idx]:
                successful_users.append(idx)
        ...
        # 4) Aggregate only successful uploads
        if len(w_locals) > 0:
        ...
        print(f"Round {r:02d} | Success: {len(successful_users)}/{args.active_UE} | Test Acc: {acc:.2f}% | Test Loss: {loss:.4f}")
